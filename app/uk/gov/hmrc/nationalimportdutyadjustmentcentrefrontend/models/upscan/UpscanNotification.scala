...
/**
  ...
  * See - https://github.com/hmrc/upscan-initiate/blob/master/README.md
  */
...
/**
  * If these checks pass, the file is made available for retrieval & the Upscan service
  * will make a POST request to the URL specified as the 'callbackUrl' by the consuming
  * service with the following body:
  *
  * @example
  * {
...
/**
  * If these checks fails, the Upscan service will make a POST request to the URL specified
  * as the 'callbackUrl' by the consuming service with the following body:
  *
  * @example
  * {
  *    "reference" : "11370e18-6e24-453e-b45a-76d3e32ea33d",
...
object UpscanNotification {
  ...
  /**
    * @param uploadTimestamp The timestamp of the file upload
  ...
  /**
    ...
    * either  QUARANTINE, REJECTED, UNKNOWN
    */
  sealed trait FailureReason

  /** The file has failed virus scanning */
  case object Quarantine extends WithValue("QUARANTINE") with FailureReason

  /** The file is not of an allowed file type */
  case object Rejected extends WithValue("REJECTED") with FailureReason

  /** There is some other problem with the file */
  case object Unknown extends WithValue("UNKNOWN") with FailureReason
  ...
  object FailureReason extends Enumerable.Implicits {
    ...
    val values: Seq[FailureReason] = Seq(Quarantine, Rejected, Unknown)

    implicit val enumerable: Enumerable[FailureReason] =

...
object RepresentationType extends Enumerable.Implicits {

  case object Importer       extends WithValue("Importer") with RepresentationType
  case object Representative extends WithValue("Representative of importer") with RepresentationType

  val values: Seq[RepresentationType] = Seq(Importer, Representative)

  def options(form: Form[_], name: String)(implicit messages: Messages): Seq[RadioItem] = values.map {
    value =>
      RadioItem(
        value = Some(value.toString),
        content = Text(MessageKey.apply("representation_type", value.toString)),
        checked = form(name).value.contains(value.toString)
      )
  }

  implicit val enumerable: Enumerable[RepresentationType] =

...
class ItemNumbersFormProvider @Inject() extends Mappings {

  def apply(): Form[ItemNumbers] = Form(
    mapping(
      "itemNumbers" -> text("itemNumbers.error.required")
        .verifying(firstError(maxLength(500, "itemNumbers.error.length")))
    )(ItemNumbers.apply)(ItemNumbers.unapply)
  )

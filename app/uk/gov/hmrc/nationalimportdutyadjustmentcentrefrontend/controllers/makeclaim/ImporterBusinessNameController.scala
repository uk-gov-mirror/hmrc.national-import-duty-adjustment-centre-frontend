...
import uk.gov.hmrc.nationalimportdutyadjustmentcentrefrontend.pages.{ImporterBusinessNamePage, Page}
...
class ImporterBusinessNameController @Inject()(
  ...
  def onPageLoad(): Action[AnyContent] = identify.async { implicit request =>
    data.getCreateAnswers map { answers =>
      val preparedForm = answers.importerBusinessName.fold(form)(form.fill)
      Ok(importerBusinessNameView(preparedForm, backLink(answers)))
    }
  }

...
import uk.gov.hmrc.nationalimportdutyadjustmentcentrefrontend.pages.{Page, RequiredDocumentsPage}
...
@Singleton
class RequiredDocumentsController @Inject() (
  mcc: MessagesControllerComponents,
  identify: IdentifierAction,
  data: CacheDataService,
  val navigator: CreateNavigator,
  view: RequiredDocumentsView
)(implicit ec: ExecutionContext)
    extends FrontendController(mcc) with I18nSupport with Navigation[CreateAnswers] {

  override val page: Page = RequiredDocumentsPage

  def onPageLoad(): Action[AnyContent] = identify.async { implicit request =>
    data.getCreateAnswers map { answers =>
      Ok(view(answers.claimType, nextPage(answers), backLink(answers)))
    }
  }

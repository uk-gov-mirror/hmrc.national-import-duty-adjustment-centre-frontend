...
class NavigatorSpec extends UnitSpec with Injector with TestData {
  ...
  "Navigator from importer eori page" when {
    val nextPage     = navigator.nextPage(ImporterHasEoriPage, _)
    val previousPage = back(ImporterHasEoriPage, _)

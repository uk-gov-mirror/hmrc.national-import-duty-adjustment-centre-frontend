...
class RichJsValueSpec extends UnitSpec with OptionValues {
  ...
  "set" when {
    ...
    "must set a value on a JsObject" in {
      ...
      val originalKey   = "some-key"
      val originalValue = "some-value"
      val pathKey       = "some-path"
    ...
    "must set nested objects and arrays" in {
      ...
      value.set(path, JsString("baz")) mustEqual JsSuccess(
        Json.obj("foo" -> Json.arr(Json.obj("bar" -> Json.arr("baz"))))
      )
    }
  }
  ...
  "remove" when {
    ...
    "must return an error if the path does not contain a value" in {
      ...
      val originalKey   = "org-key"
      val originalValue = "org-value"
    ...
    "must remove a value given a keyPathNode and return the new object" in {
      ...
      val values        = List("value1", "value2")
      val keyToRemove   = "key-to-remove"
      val valueToRemove = "vale-to-remove"
  ...
  "remove the value if the last value is deleted from an array" in {
    ...
    input.remove(path) mustBe JsSuccess(
      Json.obj("key" -> JsArray(), "key2" -> JsArray(Seq(Json.toJson(1), Json.toJson(2))))
    )
  }
}
